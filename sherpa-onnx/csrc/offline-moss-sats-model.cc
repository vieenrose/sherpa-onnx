// sherpa-onnx/csrc/offline-moss-sats-model.cc
//
// Copyright (c)  2026  zengyw

#include "sherpa-onnx/csrc/offline-moss-sats-model.h"
...
constexpr int32_t kDecoderCachePositionInputIndex = 2;
constexpr int32_t kDecoderPastKvInputStartIndex = 3;
...
class OfflineMossSatsModel::Impl {
 public:
  explicit Impl(const OfflineModelConfig &config)
      : config_(config),
        env_(ORT_LOGGING_LEVEL_ERROR, "moss-sats"),
    ...
    const auto &c = config_.moss_sats;
  ...
  Impl(Manager *mgr, const OfflineModelConfig &config)
...

    {
      ...
      if (buf.empty()) {
        SHERPA_ONNX_LOGE("Failed to read moss_sats.encoder: %s",
                         c.encoder.c_str());
        SHERPA_ONNX_EXIT(-1);
      }
    ...
    }
    {
      ...
      if (buf.empty()) {
        SHERPA_ONNX_LOGE("Failed to read moss_sats.decoder: %s",
                         c.decoder.c_str());
        SHERPA_ONNX_EXIT(-1);
      }
 ...
 private:
  ...
  void InitDecoderSession() {
    ...
    if (past_key_shape_tpl_.size() >= 2 && past_key_shape_tpl_[1] > 0) {
      ...
      max_total_len_ = config_.moss_sats.max_total_len;
    }
    if (max_total_len_ <= 0) {
      SHERPA_ONNX_LOGE(
          "Failed to determine max_total_len (past_key dim1 is dynamic or "
          "invalid; set --moss-sats-max-total-len)");
      SHERPA_ONNX_EXIT(-1);
    }
 ...
 public:
  Ort::Value ForwardEncoder(Ort::Value mel) {
    // MOSS-SATS encoder: mel (B, num_mel_bins, T) -> audio embeds (B, T', H)
    if (use_cuda_iobinding_) {
      Ort::IoBinding binding(*encoder_sess_);
      binding.BindInput(encoder_input_names_ptr_[0], mel);
    ...
    std::array<Ort::Value, 1> inputs = {std::move(mel)};
  ...
  ForwardLLM(Ort::Value input_ids, Ort::Value audio_features,
    ...
    input_names_ptr.push_back(decoder_input_names_ptr_[0]);  // input_ids
    input_names_ptr.push_back(decoder_input_names_ptr_[1]);  // audio_features
    input_names_ptr.push_back(decoder_input_names_ptr_[2]);  // cache_position
...
OfflineMossSatsModel::OfflineMossSatsModel(const OfflineModelConfig &config)
    : impl_(std::make_unique<Impl>(config)) {}

template <typename Manager>
OfflineMossSatsModel::OfflineMossSatsModel(Manager *mgr,
                                           const OfflineModelConfig &config)
    : impl_(std::make_unique<Impl>(mgr, config)) {}

OfflineMossSatsModel::~OfflineMossSatsModel() = default;

Ort::Value OfflineMossSatsModel::ForwardEncoder(Ort::Value mel) {
  return impl_->ForwardEncoder(std::move(mel));
}
...
OfflineMossSatsModel::CreateEmptyKVCache(int64_t batch) {
  return impl_->CreateEmptyKVCache(batch);
}

void OfflineMossSatsModel::ApplyKvDeltaInplace(
    std::vector<std::pair<Ort::Value, Ort::Value>> *cache_kv,
    const std::vector<std::pair<Ort::Value, Ort::Value>> &kv_delta,
    const Ort::Value &cache_position) {
...
int32_t OfflineMossSatsModel::GetMaxTotalLen() const {
  return impl_->GetMaxTotalLen();
}

OrtAllocator *OfflineMossSatsModel::Allocator() const {
  return impl_->Allocator();
}

#if __ANDROID_API__ >= 9
template OfflineMossSatsModel::OfflineMossSatsModel(
